...
async function toSvgPathIfAvailable(publicPath) {
  if (typeof publicPath !== "string") return publicPath;
  if (!publicPath.startsWith("/projects/")) return publicPath;

  const svgPath = publicPath.replace(/\.(jpg|jpeg|png|webp)$/i, ".svg");
  if (svgPath === publicPath) return publicPath;

  const diskPath = path.resolve(process.cwd(), "public", svgPath.replace(/^\//, ""));
  return (await fileExists(diskPath)) ? svgPath : publicPath;
}
...
async function main() {
  ...
  for (const doc of projectsSnap.docs) {
    const data = doc.data();
    const nextMainImage = await toSvgPathIfAvailable(data.mainImage);
    ...
    for (const block of contentBlocks) {
      ...
      if (block.type === "IMAGE") {
        const nextContentPath = await toSvgPathIfAvailable(block.content);
