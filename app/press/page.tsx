        ...
        <main className="pt-32 pb-24 bg-neutral-50 min-h-screen">
            <Container>
                <div className="text-center mb-20">
                    <h1 className="font-serif text-5xl md:text-6xl mb-6">Press</h1>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                    {pressMap.map((item, index) => (
                        <Link href={`/press/${item.slug}`} key={index} className="block group">
                            <article className="bg-white p-6 shadow-sm hover:shadow-md transition-shadow h-full">
                                <div className="relative h-48 w-full mb-6 overflow-hidden bg-neutral-100">
                                    <Image
                                        src={item.image}
                                        alt={item.title}
                                        fill
                                ...
                                <div className="space-y-3">
                                    <p className="text-xs font-bold uppercase tracking-widest text-primary">{item.publication}</p>
                                    <h3 className="font-serif text-xl leading-tight group-hover:underline decoration-1">{item.title}</h3>
